    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in state.getLegalActions(index_of_agent):
                ...
                successor_value = minimizer(successor,depth,1)[0]
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            # initialize value
            value = float('inf')
            next_index = (index_of_agent + 1) % state.getNumAgents()
            ...
            for action in state.getLegalActions(index_of_agent):
                successor = state.generateSuccessor(index_of_agent, action)
                if next_index == 0:
                    successor_value = maximizer(successor, next_depth, 0)[0]
                else:
                    successor_value = minimizer(successor, next_depth, next_index)[0]
                if successor_value < value:
                    value = successor_value
                    miniAction = action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in state.getLegalActions(index_of_agent):
                successor = state.generateSuccessor(index_of_agent,action)
                successor_value = minimizer(successor,depth,1,alpha,beta)[0]
                if successor_value > value:
                    value = successor_value
                    maxiAction = action
                if value > beta:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in state.getLegalActions(index_of_agent):
                successor = state.generateSuccessor(index_of_agent, action)
                if next_index == 0:
                    successor_value = maximizer(successor, next_depth, 0,alpha,beta)[0]
                else:
                    successor_value = minimizer(successor, next_depth, next_index,alpha,beta)[0]
                if successor_value < value:
                    value = successor_value
                    miniAction = action
                if value < alpha:
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=1):
        ...
        def Selection(cgs, cgstree):
            ...
            while True:
                if (cgstree.north == None
                    or cgstree.east == None
                    or cgstree.west == None
                    or cgstree.south == None
                    or cgstree.stop == None):
                    return cgs,cgstree
                else:
                    children = [
                        (cgstree.north,'north'),
        ...
        def Expansion(cgstree):
            ...
            for action in legal_actions:
                successor = cgstree.statevalue.generateSuccessor(0,action)
                next_node = Node((successor, 0, 1))
                next_node.parent = cgstree
                if action == 'North':
                    cgstree.north = next_node
                elif action == 'East':
                    cgstree.east = next_node
                elif action == 'West':
                    cgstree.west = next_node
                elif action == 'South':
                    cgstree.south = next_node
                else:
                    cgstree.stop = next_node
        def Simulation(cgs, cgstree):
            ...
            '''
            simulation_score = HeuristicFunction(cgs)
